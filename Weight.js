...
export function Weight(ns, server) {
  ...
  if (server.startsWith('hacknet')) { return 0; }
  ...
  if (ns.fileExists('Formulas.exe')) {
    // We use weakenTime instead of minDifficulty since we have access to it, 
    // and we add hackChance to the mix (pre-formulas.exe hack chance formula 
    // is based on current security, which is useless)
